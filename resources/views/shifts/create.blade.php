...
<div class="container mx-auto p-6">
    ...
    <form action="{{ route('shifts.store') }}" method="POST" class="bg-white p-6 rounded shadow-md">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="start_time" value="{{ old('start_time') }}" class="w-full p-2 border border-gray-300 rounded" placeholder="hh:mm AM/PM">
            @error('start_time')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <input type="text" name="end_time" value="{{ old('end_time') }}" class="w-full p-2 border border-gray-300 rounded" placeholder="hh:mm AM/PM">
            @error('end_time')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
